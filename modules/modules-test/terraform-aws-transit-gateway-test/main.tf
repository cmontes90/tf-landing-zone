locals {
  ...
  vpc_attachments_with_routes = chunklist(flatten([
    for k, v in var.vpc_attachments : setproduct([{ key = k }], v["tgw_routes"]) if length("tgw_routes") > 0
  ]), 2)
...
resource "aws_ec2_transit_gateway_route_table" "route_table_production" {
  count = var.create_tgw ? 1 : 0
...
resource "aws_ec2_transit_gateway_route_table" "route_table_non_production" {
  count = var.create_tgw ? 1 : 0
...
resource "aws_ec2_transit_gateway_route" "this" {
  count = var.create_tgw ? length(local.vpc_attachments_with_routes) : 0
...
resource "aws_ec2_transit_gateway_route" "transit_gateway_route_prod" {
  count = var.create_tgw ? length(local.vpc_attachments_with_routes) : 0
...
resource "aws_ec2_transit_gateway_route" "transit_gateway_route_non_prod" {
  count = var.create_tgw ? length(local.vpc_attachments_with_routes) : 0

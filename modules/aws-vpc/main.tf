...
resource "aws_route" "public_transit_gateway" {
  count = var.create_vpc && var.tgw_route && length(var.public_subnets) > 0 ? 1 : 0

  route_table_id         = aws_route_table.public[0].id
  destination_cidr_block = var.internal_network == null ? "0.0.0.0/0" : var.internal_network
...
resource "aws_route" "private_transit_gateway" {
  count = var.create_vpc && var.tgw_route && local.max_subnet_length > 0 ? local.nat_gateway_count : 0
...
resource "aws_route" "intra_transit_gateway" {
  count = var.create_vpc && var.tgw_route && length(var.intra_subnets) > 0 ? 1 : 0
...
resource "aws_customer_gateway" "this" {
  ...
  tags = merge(
    {
      Name = format("%s-cgw-%s", var.name, each.key)
    },
    var.tags,
